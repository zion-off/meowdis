...
export function translateDBSize(args: string[]): Translation {
    ...
    const statements: Statement[] = [
        { sql: "SELECT COUNT(*) as count FROM keys WHERE (expires_at IS NULL OR expires_at > unixepoch())", params: [] },
    ];
...
export function translateKeys(args: string[]): Translation {
    ...
    const statements: Statement[] = [
        { sql: "SELECT key FROM keys WHERE (expires_at IS NULL OR expires_at > unixepoch()) ORDER BY key", params: [] },
    ];
